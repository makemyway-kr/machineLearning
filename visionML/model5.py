...
with tf.device("/GPU:0"):
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(lr=learningRate), loss=tfa.losses.SigmoidFocalCrossEntropy() , metrics = ['accuracy',tf.keras.metrics.AUC()])
    model.summary()

    model.fit(trainImg,trainLabel,epochs = 15 , batch_size  = 10 , validation_data = (testImg,testLabel))

    model.save("model_M1_3_Densenet_FOCAL.h5")
